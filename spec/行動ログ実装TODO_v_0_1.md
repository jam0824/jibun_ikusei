...
- [ ] ブラウザ時間の正本を Chrome Extension に保ったまま、行動ログ向けイベント送出を追加する
- [ ] 閲覧クエスト系と行動ログ系の住み分けをコード上でも明確にする

### 実装対象

- [ ] 行動ログ向けの `browser_page_changed / heartbeat` 送出
- [ ] `elapsedSeconds` などのブラウザ補助メタデータの送出
- [ ] extension から desktop へ渡すイベントのフォーマット整備
- [ ] extension 無効時の粗い fallback 文脈の整理
- [ ] シークレット / プライベートブラウジング時は行動ログイベントを送出しない

### 先に書く failing test

- [ ] タブ切り替え時に行動ログイベントが送出されるテスト
- [ ] heartbeat に必要なメタデータが含まれるテスト
- [ ] 閲覧クエスト用集計と行動ログ用送出が衝突しないテスト
- [ ] extension 無効時に正確な滞在時間正本へ降格しないことのテスト
- [ ] シークレット / プライベートブラウジング中は行動ログイベントを送出しないテスト

### 実装メモ

- [ ] 既存のブラウザ時間計測ロジックを書き換えすぎない
- [ ] 行動ログ側はページ滞在時間の一次正本ではなくイベント受領側であることを守る
- [ ] desktop が再計測しない前提をテストでも押さえる

### 完了条件

- [ ] extension から行動ログ向けイベントが安定して送れる
- [ ] 閲覧クエスト系の正本時間計測を壊していない
- [ ] 行動ログ側で必要なブラウザ文脈が取得できる
- [ ] シークレット / プライベートブラウジング除外が効いている
