...
### 25-1. ホーム
- `今日のサマリー` の `今日のクリア` カードはタップ可能とする。
- タップ時は `記録` 画面へ遷移し、URL クエリ `filter=today` を付与する。
- このカードの件数はクエスト数ではなく、`active completion` の **クリア回数** を表示する。
- ヒーローエリア内のミニ統計は参照専用のままとする。

### 25-2. 記録
- 新しい独立画面は追加せず、既存の `記録` 画面をフィルタ対応して内訳表示を担わせる。
- 上部は `今日 / 今週 / すべて` の3カードを表示し、各カード自体をタップして切り替える。
- `記録` 画面を単独で開いたときの初期表示は `today` とする。
- URL クエリは以下を使用する。
  - `/records?filter=today`
  - `/records?filter=week`
  - `/records?filter=all`
- `filter` が未指定または不正値の場合は `today` として扱う。
- 一覧は選択中フィルタに一致する completion のみを表示し、並び順は `completedAt` の新しい順とする。
- `undo` 実行後は現在のフィルタ結果と件数に即時反映する。

### 25-3. 集計定義
- 集計対象は `undoneAt` を持たない `active completion` のみ。
- 表示単位はユニーククエスト数ではなく **completion 件数** とする。
- `today` は端末ローカル時刻での当日 0:00-23:59 を指す。
- `week` は **月曜始まりのカレンダー週** を指す。
- 判定基準は `createdAt` ではなく `completedAt` を用いる。
- 年跨ぎ週でも既存の週キー方針に合わせて同一週判定が崩れないこと。
