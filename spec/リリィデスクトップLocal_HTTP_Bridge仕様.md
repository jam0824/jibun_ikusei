...
## 2026-04-11 `youtube_transcript` 追記

- v1 の対応 eventType に `youtube_transcript` を追加する。
- `youtube_transcript` は会話入口へは流さず、保存専用 callback に dispatch する。
- `occurredAt` は必須で、JST RFC3339 (`+09:00`) 以外は `invalid_occurred_at` として reject する。
- payload は次を必須とする。
  - `videoId`
  - `videoUrl`
  - `videoTitle`
  ...
  - `segments` (1 件以上)
- `segments[]` の各要素は `startSeconds: number` と `text: non-empty string` を必須とする。
- 正常時レスポンスは既存 event と同じく `202 Accepted` とし、`eventType` には `youtube_transcript` を返す。
- runtime log には既存 event と同じ粒度で `eventType`, `source`, `eventId`, `receivedAt`, `result` を残す。
