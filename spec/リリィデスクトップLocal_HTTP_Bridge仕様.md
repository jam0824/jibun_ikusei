...
- Local HTTP Bridge は `lily_desktop` 内の `Activity Capture Service` が所有する localhost 入力アダプタとして扱う。
- v1 の責務は「外部イベントを検証し、内部イベント層へ正規化して渡すこと」とする。イベントは内容に応じて既存の会話入口、可聴タブ管理、将来の行動ログ入力へ接続されうる。
