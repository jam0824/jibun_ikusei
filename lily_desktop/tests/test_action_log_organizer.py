...
async def test_organize_and_sync_reads_only_today_and_yesterday_and_preserves_hidden(tmp_path):
    ...
    api_client.existing_sessions = [{**first_payload["sessions"][0], "hidden": True}]
...
async def test_organize_and_sync_preserves_hidden_when_candidate_id_changes(tmp_path):
    ...
    first_session = api_client.put_sessions_calls[0]["sessions"][0]
    ...
    second_session = api_client.put_sessions_calls[0]["sessions"][0]
    assert second_session["id"] != first_session["id"]
    assert second_session["hidden"] is True
...
@pytest.mark.asyncio
async def test_organizer_uses_ollama_batch_and_saves_sessions(tmp_path, monkeypatch):
    log_dir = tmp_path / "raw_events"
    _write_spool(
        log_dir,
        "2026-04-17",
