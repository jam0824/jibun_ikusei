...
**新しい仕様の追加や、既存仕様の変更を伴う実装を行ったときは、必ず `spec/` 内の該当する仕様書を更新すること。**

- 機能の追加・変更・削除を行った場合、対応する仕様書（`spec/*.md`）を同じ変更でアップデートする
- 該当する仕様書が存在しない場合は、新規に仕様書を作成する
- 実装と仕様書がずれた状態でコミットを終えない（コードと仕様を常に一致させる）
- バグ修正でも、仕様上の挙動が変わる場合は仕様書に反映する
