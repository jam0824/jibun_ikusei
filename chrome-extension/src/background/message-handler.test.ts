...
describe('youtube transcript relay', () => {
  beforeEach(() => {
    fetchMock.mockClear()
    vi.resetModules()
  })

  it('YOUTUBE_TRANSCRIPT_READY メッセージで popup bridge に transcript を送る', async () => {
    const { setupMessageListener } = await import('./message-handler')
    setupMessageListener()
    const callback = vi.mocked(chrome.runtime.onMessage.addListener).mock.calls[0][0]
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        method: 'POST',
        body: expect.stringContaining('"eventType":"youtube_transcript"'),
      }),
    )
  })
})
